## Utils
validate_candidate_data <- function(data, pre = FALSE){
  ...
  if(pre){
    if(any(data$c_id >0)){
      stop(paste(sum(data$c_id >0), "candidates have a positive c_id\ nc_id must be negative for the starting candidate cohort"))
    }
    if(any(data$listing_day  >0)){
      stop(paste(sum(data$listing_day >0), "candidates have a positive listing_day\ nlisting_day must be negative for the starting candidate cohort"))
    }
  }
