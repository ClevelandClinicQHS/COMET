...
spawn_donors <- function(days, daily_lambdas_center, sex_thetas,
  ...
  if(any(df_b_long$value >1)){
    ...
    v2_f <- apply(v2, 1,  function(x) tibble(hospital = rep(x[2], x[3]), recovery_day = as.numeric(x[1]))) |>
      bind_rows()
...
spawn_candidates <- function(days, daily_lambdas_center, sex_thetas, race_mat_beta, #dx_mat_beta,
  ...
  if(any(df_b$value > 1)){
    ...
    #   purrr::list_rbind()
    v2_f <- apply(v2, 1,  function(x) tibble(center = rep(x[2], x[4]), listing_day = x[1], dx_num = x[3])) |>
      bind_rows()
